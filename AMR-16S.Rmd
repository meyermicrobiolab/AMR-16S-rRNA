...
```{r, echo=FALSE}
# Start by creating phyloseq object using full dataset (low abundance ASVs are not removed). We are using phlyoseq to remove blanks and then group ASVs by Family.
...
psnb
get_taxa_unique(psnb, "Family") # 478 families
psnb_fam <- tax_glom(psnb, "Family") # group ASVs by Family
psnb_fam # 480 taxa (families) and 50 samples

# Now put Family abundances and metadata into formats for ANCOM-BC
datm <- psmelt(psnb_fam) # melt phyloseq object into dataframe
OTUdf <- reshape2::dcast(datm, Sample ~ Family, value.var = 'Abundance', fun.aggregate = sum) # cast dataframe with only columns needed
metadf<-read.table("metadata_CLEAN_noblanks.txt",sep="\t",header=T,row.names=1)
metadf <- tibble::rownames_to_column(metadf, "Sample.ID")

# Step 1: Data preprocessing

prepro = feature_table_pre_process(feature_table = OTUdf, meta_data = metadf, sample_var = "Sample.ID", group_var = NULL, out_cut = 0.05, zero_cut = 0.90, lib_cut = 1000, neg_lb = FALSE)

#zero_cut: Taxa with proportion of zeroes greater than zero_cut are not included in the analysis.
#out_cut: observations with proportion of mixture distribution less than out_cut (or greater than 1- out_cut) will be detected as outliers
