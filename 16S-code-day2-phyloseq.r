...
# siliva_nonchloronomito_out_table.txt was modified in excel into final-ASV-table.txt 
# siliva_nonchloromomito_taxa_table.txt was modified in excel ino final-taxa-table.txt 

otu <- read.table("final-ASV-table.txt",sep="\t",header=TRUE, row.names=1)
...
#flitered ASVs=289, phyla=16, classes=25, orders=57, families=93, genera=147

sample_data(psmin5)$treatment<-factor(sample_data(psmin5)$treatment,levels=c("healthy","antibiotic","diseased"))
...
psmin5 %>% 
  ...
  ord_calc() %>% 
  ord_plot(color = "treatment", shape = "type", size = 5) +
...
set.seed(1)
da_asv_analysis <- differentialTest(formula = ~ treatment,
